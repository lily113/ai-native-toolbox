...
**微信小程序已经完整跑通并在使用**——8 个主页面 + 1 个分包全部可用，云同步和 AI 教练都已上线。

| 已实现并跑通 | 持续迭代中 |
|---|---|
| ✅ **月历 + 训练记录完整闭环**（增删改查，`pages/record`） | 🔄 语音课堂笔记（微信同声传译插件） |
| ✅ **动作库与动作详情**（`pages/moves` / `pages/move`） | 🔄 云端自动同步（当前是手动点上传/拉取） |
| ✅ **里程碑时间线**（`pages/milestone`） | 🔄 统计回顾视图的深化（趋势、时长、周期的多角度分析） |
| ✅ **训练回顾页**（`pages/review`） | 🔄 动作等级考试分包（`packageExam`）的内容扩充 |
| ✅ **AI 教练对话页**（`pages/coach`，接 DeepSeek 云函数） | 🔄 AI 教练回答的持续调优 |
| ✅ **设置与数据管理**（`pages/settings`，含备份导入导出、云同步） | |
| ✅ 每周上冰目标 | |
| ✅ 云同步（openid 隔离） | |
